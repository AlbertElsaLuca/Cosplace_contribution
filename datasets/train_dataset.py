...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, class_num):
        ...
        #in the following lines, after reading the image we will resize it to half the height and weight
        ...
        try:
            ...
            raise e
          #the different size that we will use are: [(256,256),(170,170),(1024,1024),(1536,1536)]
          #256=pil_image.size[0]//2
          #170=pil_image.size[0]//3
          #1024=pil_image.size[0]*2   
          #1536=pil_image.size[0]*3
        new_h=pil_image.size[0]*1
        new_w=pil_image.size[0]*1
        tensor_image = T.functional.to_tensor(pil_image.resize((new_w,new_h)))
        assert tensor_image.shape == torch.Size([3, new_w, new_h]), \
            f"Image {image_path} should have shape [3, {new_w}, {new_h}] but has {tensor_image.shape}."
